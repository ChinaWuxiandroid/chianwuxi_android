...
public class FragmentManagers {
	...
	/***
	 * 页面的跳转 原理：1、把当前管理类实例传给跳转的fragment 2、把跳转的fragment 添加到fragment集合中 便于管理
	...
	public void IntentFragment(BaseFragment saveFragment) {
		...
		onTransaction(saveFragment, "", true);
	}

	public void IntentFragment(BaseFragment saveFragment, boolean isSave) {
		saveFragment.setManagers(this);
		addFragment(saveFragment);
		onTransaction(saveFragment, "", isSave);
	}

	/***
	 * 页面的切换 原理：1、把当前管理类实例传给跳转的fragment 2、把跳转的fragment 添加到fragment集合中 便于管理
	...
	/***
	 * 从当前页面返回 原理1、从fragment集合中清除当前fragment

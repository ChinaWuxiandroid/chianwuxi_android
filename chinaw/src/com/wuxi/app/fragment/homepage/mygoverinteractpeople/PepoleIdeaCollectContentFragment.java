...
import com.wuxi.app.engine.PepoleCollectService;
...
public class PepoleIdeaCollectContentFragment extends BaseFragment {
	...
	private PepoleIdeaCollectWrapper pepoleIdeaCollectWrapper = null;
	...
	private void setLayoutValue() {
		...
		post_summary_text.setText("\u3000\u3000"
				+ pepoleIdeaCollectWrapper.getSummary());

		begintime_text.setText(pepoleIdeaCollectWrapper.getBegintime());
		endtime_text.setText(pepoleIdeaCollectWrapper.getEndtime());
		readnum_text.setText(pepoleIdeaCollectWrapper.getReadCount());
		replynum_text.setText(String.valueOf(pepoleIdeaReplyWrapper
				.getTotalRowsAmount()));
		title_text.setText(pepoleIdeaCollectWrapper.getTitle());

		content_text.setVisibility(View.GONE);
		post_scrollview.setVisibility(View.GONE);

		post_WebView.setVisibility(View.VISIBLE);
		post_WebView.loadData(pepoleIdeaCollectWrapper.getContent(),
	...
	private void loadData() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					PepoleCollectService collectService = new PepoleCollectService(
							context);
					pepoleIdeaCollectWrapper = collectService
							.getPepoleIdeaCollectWrapper(politics.getId());

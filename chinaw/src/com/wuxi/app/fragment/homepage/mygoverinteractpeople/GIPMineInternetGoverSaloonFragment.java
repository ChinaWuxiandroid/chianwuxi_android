...
public class GIPMineInternetGoverSaloonFragment extends
		RadioButtonChangeFragment implements OnCheckedChangeListener,
		OnClickListener {
	...
	private void showMyConsultData() {
		...
		if (myconsultWrapper.isNext()) {
			...
			if (myOnlineConsultAdapter != null) {
	...
	private void switchRadionButtonStyle() {
		...
		for (int i = 0; i < gover_btn_rg.getChildCount(); i++) {
			...
			if (rb.isChecked()) {
				...
				rb.setBackgroundResource(R.drawable.goversaool_content_bt_selec);
			} else {
